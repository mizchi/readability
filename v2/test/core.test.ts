...
import { findMainContent, extractContent, isProbablyContent } from '../core';
...
describe('Core Readability Functions', () => {
  ...
  test('findMainContent - 基本的なHTML', () => {
    ...
    // IDが"content"の要素が選択されることを確認
    if (mainContent) {
      expect(mainContent.id).toBe('content');
    }
  });

  test('findMainContent - セマンティックタグ', () => {
    ...
    // articleタグまたはmainタグが選択されることを確認
    if (mainContent) {
      expect(['ARTICLE', 'MAIN']).toContain(mainContent.tagName);
    }
  });

  test('findMainContent - 複雑なHTML', () => {
    ...
    // コンテンツクラスの要素が選択されることを確認
    if (mainContent) {
      // 直接contentクラスの要素か、その親要素が選択される
      const hasContentClass = mainContent.className === 'content' || 
        mainContent.children.some((child): boolean => 
