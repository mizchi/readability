// Mozilla/Readability v3 - 階層構造に基づくARIAスナップショット生成
// AIに渡すためのページ要約を生成し、URL階層構造に基づいてリンクを分類
...
// リンク構造を階層に基づいて分析する
function analyzeLinksHierarchy(
  root: AriaNode,
  baseUrl: string,
  domain: string
) {
          ...
          if (linkUrl.startsWith("#")) {
            ...
            try {
              ...
              if (!isDomainOrSubdomain(linkDomain, domain)) {
                ...
                hierarchy = determineHierarchyRelation(
                  basePathSegments,
                  linkPathSegments
                );
              }
...
async function main() {
  ...
  // HTMLからベースURLを抽出
  const baseUrl =
    extractBaseUrlFromHtml(html) || "https://en.wikipedia.org/wiki/Mozilla";
  ...
  if (result.ariaTree) {
    // 階層構造に基づいてリンクを分析
    console.log("\n=== リンク階層構造分析 ===");
    const hierarchyAnalysis = analyzeLinksHierarchy(
      result.ariaTree.root,
      baseUrl,
      baseDomain
    );
    ...
    console.log("\n階層ごとのリンク数:");
    console.log(
      `- 内側（より深い階層）: ${hierarchyAnalysis.hierarchyCounts.inner}件`
    );
    console.log(
      `- 同レベル（同じ階層）: ${hierarchyAnalysis.hierarchyCounts.same}件`
    );
    console.log(
      `- 外側（より浅い階層）: ${hierarchyAnalysis.hierarchyCounts.outer}件`
    );
    console.log(
      `- 別パス（異なるパス体系）: ${hierarchyAnalysis.hierarchyCounts.different}件`
    );
    console.log(
      `- 外部（別ドメイン）: ${hierarchyAnalysis.hierarchyCounts.external}件`
    );
    ...
    hierarchyAnalysis.navigationMenus.forEach((menu, i) => {
      ...
      console.log(
        `   階層分布: 内側=${menu.hierarchyDistribution.inner}, 同レベル=${menu.hierarchyDistribution.same}, 外側=${menu.hierarchyDistribution.outer}, 別パス=${menu.hierarchyDistribution.different}, 外部=${menu.hierarchyDistribution.external}`
      );

      // 各階層の最初のリンクを表示
      const innerLinks = menu.links
        .filter((link) => link.hierarchy === "inner")
        .slice(0, 2);
      const outerLinks = menu.links
        .filter((link) => link.hierarchy === "outer")
        .slice(0, 2);

      if (innerLinks.length > 0) {
        console.log(
          `   内側リンク例: ${innerLinks.map((l) => l.url).join(", ")}`
        );
      }

      if (outerLinks.length > 0) {
        console.log(
          `   外側リンク例: ${outerLinks.map((l) => l.url).join(", ")}`
        );
      }
    });
    ...
    // 階層構造の要約
    console.log("\n=== 階層構造の要約 ===");
    const totalLinks = Object.values(hierarchyAnalysis.hierarchyCounts).reduce(
      (a, b) => a + b,
      0
    );
    const innerRatio = Math.round(
      (hierarchyAnalysis.hierarchyCounts.inner / totalLinks) * 100
    );
    const outerRatio = Math.round(
    ...
    const navRoles = hierarchyAnalysis.navigationMenus.map((menu) => {
      ...
      if (inner > outer && inner > same && inner > different) {
        return {
          name: menu.name,
          role: "下位階層ナビゲーション",
          count: menu.linkCount,
        };
      } else if (outer > inner && outer > same && outer > different) {
        return {
          name: menu.name,
          role: "上位階層ナビゲーション",
          count: menu.linkCount,
        };
      } else if (same > inner && same > outer && same > different) {
        return {
          name: menu.name,
          role: "同階層ナビゲーション",
          count: menu.linkCount,
        };
      } else if (different > inner && different > outer && different > same) {
        return {
          name: menu.name,
          role: "クロスナビゲーション",
          count: menu.linkCount,
