...
async function main() {
  ...
  structure.headers.forEach((header, index) => {
    ...
    if (header.contains.siteTitle) {
      console.log(`- Site Title: "${header.contains.siteTitle.text}" (h${header.contains.siteTitle.level})`);
    }
  ...
  if (structure.mainNavigation) {
    ...
    const hasDropdowns = structure.mainNavigation.items.some(item => item.children && item.children.length > 0);
    if (hasDropdowns) {
